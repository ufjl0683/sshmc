...
namespace ArcGISLib
{
    public class Wgs84Tw97Transform
    {
      ...
      static int dx = 250000;
        ...
        public static void  lonlat_To_twd97(double lon, double lat,out double twx,out double twy)
        {
            string TWD97 = "";
            lon = (lon / 180) * Math.PI;
            lat = (lat / 180) * Math.PI;
            ...
            double S = A * lat - B * Math.Sin(2 * lat) + C * Math.Sin(4 * lat) - D * Math.Sin(6 * lat) + E * Math.Sin(8 * lat);         //計算Y值   
            double K1 = S * k0;
            ...
            double y = K1 + K2 * (Math.Pow(p, 2)) + K3 * (Math.Pow(p, 4));         //計算X值 
        ...
        public static void  tw97_To_lonlat(double twx,double twy,out double longitude,out double latitude)
        //給WGS84經緯度弧度轉成TWD97坐標 
        {
            double dy = 0;
            double e = Math.Pow((1 - Math.Pow(b, 2) / Math.Pow(a, 2)), 0.5);
            twx -= dx;
            twy -= dy;        // Calculate the Meridional Arc  
            double M = twy / k0;        // Calculate Footprint Latitude   
            ...
            double fp = mu + J1 * Math.Sin(2 * mu) + J2 * Math.Sin(4 * mu) + J3 * Math.Sin(6 * mu) + J4 * Math.Sin(8 * mu);        // Calculate Latitude and Longitude  
            ...
            double D = twx / (N1 * k0);        // 計算緯度      
            double Q1 = N1 * Math.Tan(fp) / R1; double Q2 = (Math.Pow(D, 2) / 2.0);
        ...
        private static string Cal_lonlat_To_twd97(double lon, double lat)
        {
            ...
            double K5 = (k0 * nu * (Math.Pow(Math.Cos(lat), 3)) / 6.0) * (1 - Math.Pow(Math.Tan(lat), 2) + e2 * (Math.Pow(Math.Cos(lat), 2)));
            double x = K4 * p + K5 * (Math.Pow(p, 3)) + dx;
            TWD97 = x.ToString() + "," + y.ToString(); return TWD97;
        }
        private static string Cal_TWD97_To_lonlat(double x, double y)
        {
            ...
            double e = Math.Pow((1 - Math.Pow(b, 2) / Math.Pow(a, 2)), 0.5);
            x -= dx;
            y -= dy;        // Calculate the Meridional Arc  
            double M = y / k0;        // Calculate Footprint Latitude   
            double mu = M / (a * (1.0 - Math.Pow(e, 2) / 4.0 - 3 * Math.Pow(e, 4) / 64.0 - 5 * Math.Pow(e, 6) / 256.0));
            double e1 = (1.0 - Math.Pow((1.0 - Math.Pow(e, 2)), 0.5)) / (1.0 + Math.Pow((1.0 - Math.Pow(e, 2)), 0.5));
            double J1 = (3 * e1 / 2 - 27 * Math.Pow(e1, 3) / 32.0);
            double J2 = (21 * Math.Pow(e1, 2) / 16 - 55 * Math.Pow(e1, 4) / 32.0);
