...
{
    class MFCC_GENERIC : Comm.MFCC.MFCC_DataColloetBase
    {




        public MFCC_GENERIC(string mfccid, string devType, int remotePort, int notifyPort, int consolePort, string regRemoteName, Type regRemoteType)
                ...
                try
                {
                    ...
                    while (queue.Count != 0)
                    {
                        try
                        {
                            ...
                            if (Program.mfcc_generic.manager.IsContains(rpd.devName))
                                tc = (Comm.TC.TILTTC)Program.mfcc_generic.manager[rpd.devName];
                            else
        ...
        public void Repair_job(System.Object repairinfo)
        {
            ...
            try
            {
                tc = (Comm.TC.TILTTC)Program.mfcc_generic.manager[RepairInfo.devName];
