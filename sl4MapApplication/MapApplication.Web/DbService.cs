...
namespace MapApplication.Web
{
    ...
    public class DbService : LinqToEntitiesDomainService<SSHMC01Entities>
    {
        ...
        public IQueryable<VWHOSTMFCC> GetVWHOSTMFCC()
        {
            return this.ObjectContext.VWHOSTMFCC;
        }

        public void InsertVWHOSTMFCC(VWHOSTMFCC vWHOSTMFCC)
        {
            if ((vWHOSTMFCC.EntityState != EntityState.Detached))
            {
                this.ObjectContext.ObjectStateManager.ChangeObjectState(vWHOSTMFCC, EntityState.Added);
            }
            else
            {
                this.ObjectContext.VWHOSTMFCC.AddObject(vWHOSTMFCC);
            }
        }

        public void UpdateVWHOSTMFCC(VWHOSTMFCC currentVWHOSTMFCC)
        {
            this.ObjectContext.VWHOSTMFCC.AttachAsModified(currentVWHOSTMFCC, this.ChangeSet.GetOriginal(currentVWHOSTMFCC));
        }

        public void DeleteVWHOSTMFCC(VWHOSTMFCC vWHOSTMFCC)
        {
            if ((vWHOSTMFCC.EntityState != EntityState.Detached))
            {
                this.ObjectContext.ObjectStateManager.ChangeObjectState(vWHOSTMFCC, EntityState.Deleted);
            }
            else
            {
                this.ObjectContext.VWHOSTMFCC.Attach(vWHOSTMFCC);
                this.ObjectContext.VWHOSTMFCC.DeleteObject(vWHOSTMFCC);
            }
        }
