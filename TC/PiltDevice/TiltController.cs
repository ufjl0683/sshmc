...
{
    public class TiltController : Comm.Controller.SSHDataController
    {
    
       // SensorBase[] devices;
      
        public TiltController(ControllerConfigBase config,PropertyBag propertybag)
       ...
       protected override SensorBase CreateDevice(SensorConfigBase config)
        {
            ...
            if (config.com_type == "TCP")
            {
                    ...
                    return new TiltDevice(config.id,this, config.device_name, new System.Net.IPEndPoint(System.Net.IPAddress.Parse(config.ip_comport), config.port_baud));
            }
            else  //COM
            {
               
                return new TiltDevice(config.id,this, config.device_name, config.ip_comport, config.port_baud);
            }
        ...
        protected override void OnOneMinTmrTask()
        {
            try
            {
                ...
                {
                    values[i * 3] =((TiltDevice) devices[i]).GetValue(0);
                    values[i * 3 + 1] =((TiltDevice) devices[i]).GetValue(1);
                    values[i * 3+ 2] = ((TiltDevice)devices[i]).GetValue(2);
                    //values[i * 3 + 2] = 0;
