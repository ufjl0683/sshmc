...
namespace MatrixLibrary
{
	...
	public class Matrix
	{
        ...
        public static void QR(double[,] Mat, out double[,] Q, out double[,] R)
        {
          // double qraux[6],vtmp[30],ctmp;
          //int jpvt[6];//
            double [] qraux, vtmp;
            double ctmp;
            int[] jpvt;
            ...
            qraux = new double[cols];
            vtmp = new double[rows];
            jpvt = new int[cols];
         ...
         double [,] qlin=new double[rows,rows];
            Q=qlin;
 ...
 //計算R矩陣
       //  sqrdc(xlin,nlin,plin,qraux,jpvt);
            sqrdc(Mat,qraux,jpvt,R);
        // 計算Q矩陣
         for (int i = 0; i <  cols; i ++)	
         {
          ...
          for (int j =i+1 ; j <  rows; j ++)	
          {
              ...
              for (int j = 0; j <  rows; j ++)	
              {
               ...
               {
                //ctmp=ctmp+vtmp[k]*qlin[j][k];
                   ctmp = ctmp + vtmp[k] * qlin[j,k];
               }
                   ctmp=ctmp/qraux[i];
                   for (int k = i; k <  rows; k ++)	
                   {
                    qlin[j,k]=qlin[j,k]-ctmp*vtmp[k];
                   }
              }
          }
        ...
        private static void sqrdc(double[,] xlin, double[] qraux, int[] jpvt, double[,] R)
        {
            int jp,lp1,lup,maxj,pl,pu;
 //int * iary; //iary: 2段各為 ;
        double maxnrm,tt,nrmxl,tlin,wtmp,qtmp;
 //long double * rary; //rary: 3段各為 ;

           
 //C++之nxn矩陣行列編號從0~n-1，FORTRAN之nxn矩陣行列編號從1~n，因為是從FORTRAN版改編，為了避免程式內部增加或遺漏+1或-1，因此維持原編號，
 //所以陣列輸入時
 //xlin[ldx][plin]:輸入：二維陣列，因次為(ldx，plin)，為用來作 QR 分解的矩陣。
 //    輸出：它的上三角部分存著QR因子化形式的 R 上三角矩陣，而主對角線以下的部分則存入一些資料用來推求正交矩陣。
 //nlin:X矩陣的列數。
 //plin:X矩陣的行數。
 //jpvt:輸入：一維整數陣列，長度為 plin，用來控制換軸，若須換軸，則xlin矩陣的第 K 行將以下列三種方式作置換。 
 //           若jpvt(K)>0    xlin(K)為初始行。
 //             jpvt(K)=0    xlin(K)為自由行。
 //             jpvt(K)<0    xlin(K)為末後行。
 //           在分解過程之前，初始行以及末後行的元素都被置換到xlin矩陣的初始和後面部分，然後只有自由元素才作移轉，如果 job=0 則 jpvt 就不被使用。
 //     輸出：jpvt(J)存著移到第 J 行之行向量原先所在的位址因此 SQRDC 並不計算矩陣xlin的 QR 分解形式，而是計算由
 //           第jpvt(J)所指向行向量所構成之矩陣。
 // 
 //qraux[plin]:一維陣列， 因次為 plin， 包含著更進一步的資料用來恢復分解形式的正交部分。

 //cout << "nlin="<<nlin<<endl;
 //cout << "plin="<<plin<<endl;
 //
 //step2:進行xlin矩陣之householder reduction
 //
        int nlin, plin;
        nlin = xlin.GetLength(0);
        plin = xlin.GetLength(1);

        for (int row = 0; row < nlin; row++)
            for (int col = 0; col < plin; col++)
                R[row, col] = xlin[row,col];

     //  lup = min(nlin,plin);//原函數min0
    lup = Math.Min(nlin,plin);
 for (int l = 0; l <  lup; l ++)
 {
  qraux[l] = 0.0e0;
  if (l!=nlin)
  {
   //
   //step2-2:計算第l行之householder transformation
   //
   //nrmxl = snrm2(nlin-l+1,xlin[l][l],1);
   nrmxl=0.0e0;
   for (int k = l; k <  nlin; k ++)	
   {
    nrmxl=nrmxl+R[k,l]*R[k,l];
   }
    // nrmxl=sqrt(nrmxl);
      nrmxl=Math.Sqrt(nrmxl);

   //if (abs(nrmxl)>1.0e-6)
   if (nrmxl!=0.0e0)
   {
    if (R[l,l]!=0.0e0) //原式if (x(l,l) .ne. 0.0e0) nrmxl = sign(nrmxl,x(l,l))
	{	
	    // nrmxl = abs(nrmxl)*xlin[l][l]/abs(xlin[l][l]);
        nrmxl = Math.Abs(nrmxl)*R[l,l]/Math.Abs(R[l,l]);
	}
    //sscal(nlin-l+1,1.0e0/nrmxl,xlin[l][l],1);
    for (int k = l; k <  nlin; k ++)	
    {
     R[k,l]=R[k,l]/nrmxl;
    }
    ...
    R[l,l] = 1.0e0 + R[l,l];
    //
    //step2-2-1:剩下行進行轉換;更新norm
    //
	lp1 = l + 1;
    if (plin >= lp1)
	{
     for (int j = lp1; j < plin; j ++)
     {
      //tlin = -sdot(nlin-l+1,x[l][l],1,x[l][j],1)/x[l][l];
      tlin=0;
      for (int k = l; k < nlin; k ++)	
      {
       tlin=tlin+R[k,l]*R[k,j];
      }
	  tlin=-tlin/R[l,l];
	  //
	  //saxpy(nlin-l+1,tlin,x[l][l],1,x[l][j],1);
      for (int k = l; k < nlin; k ++)	
      {
       R[k,j]=R[k,j]+tlin*R[k,l];
      }
	  //
	 }
	}
